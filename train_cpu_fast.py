...
# =======================
# Функции для Consistency Distillation на CPU (ускоренная версия)
# =======================
...
def consistency_distillation_step_cpu_fast(latents, text_embeddings, teacher_model, student_model):
    """
    Один шаг Consistency Distillation на CPU (ускоренная версия)
    """
...
# =======================
# Тренировочный цикл (ускоренная версия)
# =======================
...
def train_consistency_distillation_cpu_fast(dataloader, teacher_model, student_model, optimizer, num_epochs=5):
    """
    Обучение студента через Consistency Distillation на CPU (ускоренная версия)
    """
    losses_history = []
    
    print(f"\n{'='*60}")
    print(f"Начало обучения на CPU (УСКОРЕННАЯ ВЕРСИЯ): {num_epochs} эпох, {len(dataloader)} батчей на эпоху")
    print(f"🚀 ОПТИМИЗАЦИИ: batch_size=1, num_workers=0, float32, 5 эпох")
    print(f"⏱️  Примерное время: ~{len(dataloader) * 1.5 / 3600:.1f} часов на эпоху")
    print(f"{'='*60}\n")
    ...
    for epoch in range(num_epochs):
        ...
        for i, (latents, prompts) in enumerate(dataloader):
            try:
                ...
                loss = consistency_distillation_step_cpu_fast(
                    latents, text_embeddings, teacher_model, student_model
                )
                
                # Проверяем на NaN и выводим отладочную информацию
                if torch.isnan(loss):
                    print(f"⚠️ NaN loss в батче {i}")
                    ...
                    print(f"   teacher_denoised shape: {teacher_denoised.shape if 'teacher_denoised' in locals() else 'N/A'}")
                    print(f"   student_denoised shape: {student_denoised.shape if 'student_denoised' in locals() else 'N/A'}")
                    continue
                ...
                # Логирование
                if i % 20 == 0:  # Реже логируем для ускорения
...
if __name__ == "__main__":
    print("🖥️  Запуск Consistency Distillation на CPU (УСКОРЕННАЯ ВЕРСИЯ)")
    print("🚀 ОПТИМИЗАЦИИ: batch_size=4, num_workers=0, float32, 5 эпох")
    print("⏱️  Примерное время: ~5-8 часов (вместо 22 часов)")
    ...
    try:
        teacher_model = create_latent_diffusion(
            ...
            dtype="float32"  # Используем float32 для стабильности на CPU
        )
        ...
        teacher_model = create_latent_diffusion(
            ...
            dtype="float32"  # Fallback на float32
        ).to("cpu")
    ...
    # Создаем датасет и даталоадер (УСКОРЕННАЯ ВЕРСИЯ)
    ...
    # Запускаем обучение (УСКОРЕННАЯ ВЕРСИЯ)
    num_epochs = 5  # Уменьшили количество эпох
    print(f"🚀 Начинаем обучение на {num_epochs} эпох...")
    print(f"⏱️  Примерное время: ~{len(dataloader) * 1.5 * num_epochs / 3600:.1f} часов")
    
    losses = train_consistency_distillation_cpu_fast(
        dataloader, teacher_model, student_model, optimizer, num_epochs
    )
    
    # Сохраняем финальную модель
    final_model_path = "student_final_cpu_fast.pt"
    ...
    plt.title('Consistency Distillation Training Loss (CPU Fast)', fontsize=14)
    plt.grid(True, alpha=0.3)
    plt.savefig('training_loss_cpu_fast.png', dpi=150, bbox_inches='tight')
    print(f"📊 График сохранен: training_loss_cpu_fast.png")
