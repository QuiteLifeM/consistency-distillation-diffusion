...
def train_consistency_distillation(dataloader, teacher_model, student_model, optimizer, num_epochs=10):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (latents, prompts) in enumerate(dataloader):
            try:
                # ОСТАВЛЯЕМ ДАННЫЕ НА CPU! Перемещаем на GPU только когда нужно
                latents = latents.float()  # Остается на CPU
                ...
                # Получаем текстовые эмбеддинги (остаются на CPU)
                text_embeddings = get_text_embeddings(prompts, teacher_model)
                
                # Перемещаем данные на GPU только для вычислений
                latents_gpu = latents.to("cuda")
                text_embeddings_gpu = text_embeddings.to("cuda")
                
                # Consistency distillation step
                loss = consistency_distillation_step(
                    latents_gpu, text_embeddings_gpu, teacher_model, student_model
                )
                ...
                del latents_gpu, text_embeddings_gpu
