...
# =======================
# Функции для Consistency Distillation с маленькой моделью студента
# =======================
...
def consistency_distillation_step_small_student(latents, text_embeddings, teacher_model, student_model):
    """
    Один шаг Consistency Distillation с маленькой моделью студента
    """
    ...
    def student_forward():
        return student_model.model_forward_wrapper(
            noisy_latents.float(),
            sigma,
    ...
    # Применяем gradient checkpointing
    student_output = torch.utils.checkpoint.checkpoint(
        student_forward,
        use_reentrant=False  # Более стабильная версия
    )
...
def train_consistency_distillation_small_student(dataloader, teacher_model, student_model, optimizer, num_epochs=10):
    """
    Обучение студента через Consistency Distillation с маленькой моделью
    """
    losses_history = []
    
    print(f"\n{'='*60}")
    print(f"Начало обучения с маленькой моделью студента: {num_epochs} эпох, {len(dataloader)} батчей на эпоху")
    print(f"{'='*60}\n")
    ...
    for epoch in range(num_epochs):
        ...
        for i, (latents, prompts) in enumerate(dataloader):
            try:
                ...
                # Consistency distillation step с маленькой моделью
                loss = consistency_distillation_step_small_student(
...
if __name__ == "__main__":
    ...
    print("\nСоздание МАЛЕНЬКОЙ модели студента...")
    # Создаем меньшую модель студента для экономии памяти
    ...
    print("✅ Маленькая модель студента создана и инициализирована весами учителя")
    ...
    # Запускаем обучение
    num_epochs = 10
    losses = train_consistency_distillation_small_student(
    ...
    plt.title('Consistency Distillation Training Loss (Small Student)', fontsize=14)
