...
# =======================
# Функции для Consistency Distillation с Gradient Checkpointing
# =======================
...
def consistency_distillation_step_with_checkpointing(latents, text_embeddings, teacher_model, student_model):
    """
    Один шаг Consistency Distillation с Gradient Checkpointing
    """
...
# =======================
# Тренировочный цикл с Micro-batching
# =======================
...
def train_consistency_distillation_with_checkpointing(dataloader, teacher_model, student_model, optimizer, num_epochs=10):
    """
    Обучение студента через Consistency Distillation с Gradient Checkpointing
    """
    losses_history = []
    
    print(f"\n{'='*60}")
    print(f"Начало обучения с Gradient Checkpointing: {num_epochs} эпох, {len(dataloader)} батчей на эпоху")
    print(f"{'='*60}\n")
    ...
    for epoch in range(num_epochs):
        ...
        for i, (latents, prompts) in enumerate(dataloader):
            try:
                ...
                # Consistency distillation step с gradient checkpointing
                loss = consistency_distillation_step_with_checkpointing(
...
if __name__ == "__main__":
    ...
    # Запускаем обучение
    num_epochs = 10
    losses = train_consistency_distillation_with_checkpointing(
    ...
    plt.title('Consistency Distillation Training Loss (with Gradient Checkpointing)', fontsize=14)
