...
# =======================
# Функции для Consistency Distillation на CPU
# =======================
...
def consistency_distillation_step_cpu(latents, text_embeddings, teacher_model, student_model):
    """
    Один шаг Consistency Distillation на CPU
    """
...
def train_consistency_distillation_cpu(dataloader, teacher_model, student_model, optimizer, num_epochs=10):
    """
    Обучение студента через Consistency Distillation на CPU
    """
    losses_history = []
    
    print(f"\n{'='*60}")
    print(f"Начало обучения на CPU: {num_epochs} эпох, {len(dataloader)} батчей на эпоху")
    print(f"⚠️  ВНИМАНИЕ: CPU обучение медленное (~2-4 сек/батч)")
    print(f"⏱️  Примерное время: ~{len(dataloader) * 3 / 3600:.1f} часов на эпоху")
    print(f"{'='*60}\n")
    ...
    for epoch in range(num_epochs):
        ...
        for i, (latents, prompts) in enumerate(dataloader):
            try:
                ...
                loss = consistency_distillation_step_cpu(
...
if __name__ == "__main__":
    print("🖥️  Запуск Consistency Distillation на CPU")
    print("⚠️  ВНИМАНИЕ: CPU обучение медленное, но стабильное")
    ...
    try:
        teacher_model = create_latent_diffusion(
            ...
            dtype="float32"  # Используем float32 для CPU (float16 может быть медленнее)
        )
    ...
    student_model = create_latent_diffusion(
        ...
        pos_interp_scale=2.0,
        precomputed_latents=False,
        dtype="float32"  # Используем float32 для CPU
    ).to("cpu")
    ...
    print(f"⏱️  Примерное время: ~{len(dataloader) * 3 * num_epochs / 3600:.1f} часов")
    
    losses = train_consistency_distillation_cpu(
        dataloader, teacher_model, student_model, optimizer, num_epochs
    )
    
    # Сохраняем финальную модель
    final_model_path = "student_final_cpu.pt"
    ...
    plt.title('Consistency Distillation Training Loss (CPU)', fontsize=14)
    plt.grid(True, alpha=0.3)
    plt.savefig('training_loss_cpu.png', dpi=150, bbox_inches='tight')
    print(f"📊 График сохранен: training_loss_cpu.png")
