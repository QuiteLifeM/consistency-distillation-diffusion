...
# =======================
# Функции для Consistency Distillation с максимальной оптимизацией
# =======================
...
def consistency_distillation_step_ultra_optimized(latents, text_embeddings, teacher_model, student_model):
    """
    Один шаг Consistency Distillation с максимальной оптимизацией памяти
    """
...
def train_consistency_distillation_ultra_optimized(dataloader, teacher_model, student_model, optimizer, num_epochs=10):
    """
    Обучение студента через Consistency Distillation с максимальной оптимизацией
    """
    losses_history = []
    
    print(f"\n{'='*60}")
    print(f"Начало обучения с максимальной оптимизацией: {num_epochs} эпох, {len(dataloader)} батчей на эпоху")
    print(f"{'='*60}\n")
    ...
    for epoch in range(num_epochs):
        ...
        for i, (latents, prompts) in enumerate(dataloader):
            try:
                ...
                # Consistency distillation step с максимальной оптимизацией
                loss = consistency_distillation_step_ultra_optimized(
...
if __name__ == "__main__":
    ...
    # Запускаем обучение
    num_epochs = 10
    losses = train_consistency_distillation_ultra_optimized(
    ...
    plt.title('Consistency Distillation Training Loss (Ultra Optimized)', fontsize=14)
