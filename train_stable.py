...
# =======================
# Функции для Consistency Distillation на CPU (стабильная версия)
# =======================
...
def consistency_distillation_step_stable(latents, text_embeddings, teacher_model, student_model):
    """
    Один шаг Consistency Distillation на CPU (стабильная версия)
    """
...
# =======================
# Тренировочный цикл (стабильная версия)
# =======================
...
def train_consistency_distillation_stable(dataloader, teacher_model, student_model, optimizer, num_epochs=5):
    """
    Обучение студента через Consistency Distillation на CPU (стабильная версия)
    """
    losses_history = []
    
    print(f"\n{'='*60}")
    print(f"Начало обучения на CPU (СТАБИЛЬНАЯ ВЕРСИЯ): {num_epochs} эпох, {len(dataloader)} батчей на эпоху")
    print(f"🔧 СТАБИЛЬНОСТЬ: batch_size=1, num_workers=0, float32, без multiprocessing")
    print(f"⏱️  Примерное время: ~{len(dataloader) * 2.5 / 3600:.1f} часов на эпоху")
    print(f"{'='*60}\n")
    ...
    for epoch in range(num_epochs):
        ...
        for i, (latents, prompts) in enumerate(dataloader):
            try:
                ...
                loss = consistency_distillation_step_stable(
                ...
                # Логирование
                if i % 50 == 0:  # Реже логируем для стабильности
        ...
        if num_batches > 0:
            ...
            # Сохраняем чекпоинт каждую эпоху для стабильности
...
if __name__ == "__main__":
    print("🖥️  Запуск Consistency Distillation на CPU (СТАБИЛЬНАЯ ВЕРСИЯ)")
    print("🔧 СТАБИЛЬНОСТЬ: batch_size=1, num_workers=0, float32, без multiprocessing")
    print("⏱️  Примерное время: ~12-15 часов (стабильно)")
    ...
    # Создаем датасет и даталоадер (СТАБИЛЬНАЯ ВЕРСИЯ)
    print("Загрузка данных...")
    dataset = LatentPromptDataset(latents_dir, prompts_dir)
    dataloader = DataLoader(
        dataset, 
        batch_size=1,  # Минимальный размер для стабильности
        shuffle=True, 
        num_workers=0,  # Отключаем multiprocessing
        collate_fn=custom_collate
    )
    print(f"✅ DataLoader создан: {len(dataset)} сэмплов, batch_size=1, num_workers=0\n")
    
    # Запускаем обучение (СТАБИЛЬНАЯ ВЕРСИЯ)
    num_epochs = 5
    print(f"🚀 Начинаем обучение на {num_epochs} эпох...")
    print(f"⏱️  Примерное время: ~{len(dataloader) * 2.5 * num_epochs / 3600:.1f} часов")
    
    losses = train_consistency_distillation_stable(
        dataloader, teacher_model, student_model, optimizer, num_epochs
    )
    
    # Сохраняем финальную модель
    final_model_path = "student_final_stable.pt"
    ...
    plt.title('Consistency Distillation Training Loss (Stable)', fontsize=14)
    plt.grid(True, alpha=0.3)
    plt.savefig('training_loss_stable.png', dpi=150, bbox_inches='tight')
    print(f"📊 График сохранен: training_loss_stable.png")
