        ...
        for model_path in pretrained_models:
            if os.path.exists(model_path):
                ...
                try:
                    ...
                    print(f"Несовместимость размеров для {os.path.basename(model_path)}: {e}")
                    continue
        
        if not teacher_loaded:
            print("Предобученные веса Teacher не найдены или несовместимы, используем случайную инициализацию")
            print("Teacher будет обучен с нуля - это нормально для CD!")
            
    except Exception as e:
        print(f"Ошибка загрузки Teacher весов: {e}")
