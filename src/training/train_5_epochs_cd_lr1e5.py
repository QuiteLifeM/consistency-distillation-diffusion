...
def log_memory_usage(iteration, epoch, prefix=""):
    ...
    print(f"     RAM: {mem_info['ram_used_gb']:.1f}/{mem_info['ram_total_gb']:.1f} GB ({mem_info['ram_percent']:.1f}%)")
    ...
    if mem_info['vram_percent'] > 90:
        print(f"     ВНИМАНИЕ: VRAM > 90%! Возможен OOM!")
    if mem_info['ram_percent'] > 80:
        print(f"     ВНИМАНИЕ: RAM > 80%! Возможны проблемы!")
