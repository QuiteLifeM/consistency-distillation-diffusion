#!/usr/bin/env python3
"""
🚀 ПОЛНОЕ обучение True Consistency Distillation на 5 эпох с lr=1e-5
======================================================================
На основе рабочей версии test_true_cd_20_iters.py:
- 5 эпох обучения
- batch_size=1 (безопасно)
- lr=1e-5 (более стабильное обучение)
...
def get_memory_info():
    ...
    if torch.cuda.is_available():
        ...
        vram_allocated = vram_reserved = vram_total = vram_percent = 0
...
def log_memory_usage(iteration, epoch, prefix=""):
    ...
    print(f"{prefix}💾 Память (итерация {iteration}, эпоха {epoch}):")
    print(f"   🖥️  RAM: {mem_info['ram_used_gb']:.1f}/{mem_info['ram_total_gb']:.1f} GB ({mem_info['ram_percent']:.1f}%)")
    print(f"   🎮 VRAM: {mem_info['vram_allocated_gb']:.1f}/{mem_info['vram_total_gb']:.1f} GB ({mem_info['vram_percent']:.1f}%)")
    print(f"   📊 VRAM зарезервировано: {mem_info['vram_reserved_gb']:.1f} GB")
    
    # Предупреждения
    if mem_info['vram_percent'] > 90:
        print(f"   ⚠️  ВНИМАНИЕ: VRAM > 90%! Возможен OOM!")
    if mem_info['ram_percent'] > 80:
        print(f"   ⚠️  ВНИМАНИЕ: RAM > 80%! Возможны проблемы!")
...
def train_5_epochs_consistency_distillation(dataloader, teacher_model, student_model, optimizer, num_epochs=5):
    ...
    print(f"🎯 Цель: {num_epochs} эпох, batch_size=1, lr=1e-5")
    print(f"📊 Сохранение чекпоинтов каждую эпоху")
    print(f"⏰ Начало: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{'='*80}\n")
    
    # Начальная информация о памяти
    log_memory_usage(0, 0, "🚀 НАЧАЛО ОБУЧЕНИЯ:")
    ...
    for epoch in range(num_epochs):
        ...
        for latents, prompts in pbar:
            try:
                ...
                # Мониторинг памяти каждые 50 итераций
                if iter_count % 50 == 0:
                    log_memory_usage(iter_count, epoch + 1, "📊 ПРОМЕЖУТОЧНЫЙ МОНИТОРИНГ:")
                ...
                # Логирование памяти при ошибке
                log_memory_usage(iter_count, epoch + 1, "❌ ОШИБКА:")
                continue
        ...
        # Финальный мониторинг памяти после эпохи
        log_memory_usage(iter_count, epoch + 1, "🏁 ЗАВЕРШЕНИЕ ЭПОХИ:")
        
        # Сохранение чекпоинта эпохи
        checkpoint_path = f"student_epoch_{epoch + 1}_lr1e5.pt"
        ...
        plt.title(f'Consistency Distillation Loss (эпоха {epoch + 1}, lr=1e-5)')
        plt.xlabel('Итерация')
        plt.ylabel('Loss')
        plt.grid(True, alpha=0.3)
        plt.savefig(f'loss_epoch_{epoch + 1}_lr1e5.png', dpi=150, bbox_inches='tight')
        plt.close()
        print(f"   📊 График сохранен: loss_epoch_{epoch + 1}_lr1e5.png")
...
def main():
    print("🚀 ПОЛНОЕ обучение True Consistency Distillation на 5 эпох с lr=1e-5")
    print("=" * 80)
    ...
    # Optimizer с lr=1e-5
    optimizer = optim.SGD(student_model.dit.parameters(), lr=1e-5, momentum=0.9)
    print("✅ Optimizer создан с lr=1e-5")
    ...
    # Информация о памяти перед началом обучения
    log_memory_usage(0, 0, "🚀 ПЕРЕД НАЧАЛОМ ОБУЧЕНИЯ:")
    
    # ПОЛНОЕ ОБУЧЕНИЕ НА 5 ЭПОХ
    print("\n🚀 Запуск полного обучения (5 эпох, lr=1e-5)...")
    losses = train_5_epochs_consistency_distillation(dataloader, teacher_model, student_model, optimizer, num_epochs=5)
    
    # Финальное сохранение
    final_checkpoint_path = "student_final_5epochs_lr1e5.pt"
    ...
    plt.title('Полное обучение True Consistency Distillation (5 эпох, lr=1e-5)', fontsize=16)
    plt.xlabel('Итерация', fontsize=14)
    plt.ylabel('Loss', fontsize=14)
    plt.grid(True, alpha=0.3)
    plt.savefig('final_loss_5epochs_lr1e5.png', dpi=300, bbox_inches='tight')
    print("📊 Финальный график: final_loss_5epochs_lr1e5.png")
    ...
    os.makedirs("final_5epochs_lr1e5_outputs", exist_ok=True)
    
    for i, prompt in enumerate(test_prompts):
        print(f"  {i+1}. '{prompt}'")
        image = generate_image(student_model, prompt, num_steps=4, device="cuda")
        output_path = f"final_5epochs_lr1e5_outputs/test_{i+1}.png"
        image.save(output_path)
        print(f"     ✅ {output_path}")
    
    # Финальный мониторинг памяти
    log_memory_usage(len(losses), 5, "🏁 ФИНАЛЬНЫЙ МОНИТОРИНГ:")
    
    print("\n" + "=" * 80)
    print("✅ ПОЛНОЕ ОБУЧЕНИЕ НА 5 ЭПОХ С lr=1e-5 ЗАВЕРШЕНО!")
    ...
    print(f"📊 Финальный график: final_loss_5epochs_lr1e5.png")
    print(f"🎨 Тестовые изображения: final_5epochs_lr1e5_outputs/")
    print("=" * 80)
